...
function ai_call_api_translations($text, $url){
    $url .= '?q='.$text;
    $curl = curl_init();
...
function ai_api_translations($data){
    $settings       = Loco_data_Settings::get();
    $com_api_key    = (array)$settings['com_api_key'];
    $url            = $com_api_key[0];
    $data_new       = '';
    foreach ($data as $key => $text) {
        $t1 = ai_call_api_translations(urlencode($text), $url);
        $t2 = json_decode($t1, true);
        $t4 = $t2['string'];
        $data_new .= $t4."(*)";
    }
    $data_new = trim($data_new, '(*)');
    $data_ar = explode('(*)', $data_new);
    return $data_ar;
}
add_filter('loco_api_translate_com', 'ai_api_translations');