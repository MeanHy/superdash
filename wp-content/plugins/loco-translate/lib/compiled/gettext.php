 ...
 * @noinspection ALL
 */
interface LocoArrayInterface extends
    ArrayAccess,
    Iterator,
    Countable,
    JsonSerializable
...
function loco_normalize_charset($cs)
{
    ...
    if (false === $aliases) {
        throw new InvalidArgumentException(
            "Unsupported character encoding: " . $cs
        );
    }
    if (preg_grep('/^ISO[-_]\\d+[-_]\\d+$/i', $aliases)) {
        $cs = current($aliases);
        $cs = strtr(strtoupper($cs), "_", "-");
    } elseif (in_array("US-ASCII", $aliases, true)) {
        $cs = "US-ASCII";
    }
    return $cs;
}
class LocoPoHeaders extends LocoHeaders
{
    ...
    public static function fromMsgstr($str)
    {
        ...
        if (
            "" !== $cs &&
            "UTF-8" !== $cs &&
            "UTF-8" !== mb_detect_encoding($str, ["UTF-8", $cs], true)
        ) {
...
function loco_convert_utf8($str, $enc, $strict)
{
    if ("" === $enc || "UTF-8" === $enc || "US-ASCII" === $enc) {
        if (false === preg_match("//u", $str)) {
            if ($strict) {
                $e = new Loco_error_ParseException(
                    $enc
                        ? "Invalid " . $enc . " encoding"
                        : "Unknown character encoding"
                );
                if (
                    preg_match(
                        '/^(?:[\\x00-\\x7F]|[\\xC0-\\xDF][\\x80-\\xBF]|[\\xE0-\\xEF][\\x80-\\xBF]{2}|[\\xF0-\\xFF][\\x80-\\xBF]{3})*/',
                        $str,
                        $r
                    ) &&
                    $str !== $r[0]
...
class LocoPoParser extends LocoGettextParser implements Iterator
{
        ...
        while (array_key_exists(++$i, $this->lines)) {
            ...
            try {
                ...
                if ("#" === $c) {
                    ...
                    $f = $line[1];
                    $entry["#"][$f][] = trim(
                        substr($line, 1 + strlen($f)),
                        "/ \n\r\t"
                    );
                } elseif (
                    preg_match(
                        '/^msg(id(?:_plural)?|ctxt|str(?:\\[(\\d+)])?)[ \\t]*/',
                        $line,
                        $r
                    )
                ) {
    ...
    public function parse($limit = -1)
    {
        ...
        if (
            ...
            $head = $this->setHeader(
                LocoPoHeaders::fromMsgstr($entry["str"][0])
            );
        }
        if (0 === $limit) {
            return [];
        ...
        while ($this->valid()) {
            ...
            if (isset($entry["id"][1])) {
                ...
                while (++$idx < $num) {
                    $plural = [
                        "source" => "",
                        "target" => isset($entry["str"][$idx])
                            ? $this->str($entry["str"][$idx])
                            : "",
                        "plural" => $idx,
                        "parent" => $pidx,
                    ];
...
class LocoMoParser extends LocoGettextParser
{
    ...
    public function parse($limit = -1)
    {
        ...
        while ($soffset < $targetOffset) {
            ...
            if (isset($sources[1])) {
                ...
                for ($n = 1; $n < $nforms; $n++) {
                    $r[++$i] = [
                        "source" => isset($sources[$n])
                            ? $this->str($sources[$n])
                            : sprintf("%s (plural %u)", $r[$p]["source"], $n),
                        "target" => isset($targets[$n])
                            ? $this->str($targets[$n])
                            : "",
                        "parent" => $p,
                        "plural" => $n,
    ...
    private function bytes($offset, $length)
    {
        ...
        if (strlen($s) !== $length) {
            throw new Loco_error_ParseException(
                "Failed to read " . $length . " bytes at [" . $offset . "]"
            );
        }
        return $s;
    }
    ...
    private function integerAt($offset)
    {
        ...
        if (!isset($arr[1]) || !is_int($arr[1])) {
            throw new Loco_error_ParseException(
                "Failed to read integer at byte " . $offset
            );
        }
        return $arr[1];
    }
    ...
    public function parse($limit = -1)
    {
        ...
        foreach ($this->ld as $messages) {
            if (!is_array($messages)) {
                throw new Loco_error_ParseException("Array expected");
            }
            $msgid = key($messages);
            if ("" === $msgid) {
...
class LocoJedHeaders extends LocoPoHeaders
{
    ...
    {
        foreach (
            ["Language" => "lang", "plural_forms" => "Plural-Forms"]
            as $canonical => $alias
        ) {
            if (
                array_key_exists($alias, $raw) &&
                !array_key_exists($canonical, $raw)
            ) {
...
abstract class LocoPo
{
    public static function pair(
        ...
        if ($nbr) {
            ...
            $width &&
            $width < mb_strlen($text, "UTF-8") + strlen($key) + 3
        ) {
        } else {
            return $key . ' "' . $text . '"';
        }
            ...
            foreach (explode($eol, $text) as $unwrapped) {
                ...
                while ($length > $width) {
                    if (preg_match($a, $unwrapped, $r)) {
                        $line = $r[0];
                    } elseif (preg_match($b, $unwrapped, $r)) {
                        $line = $r[0];
                    } else {
    ...
    public function merge(LocoPoMessage $def, $translate = false)
    {
        if ($def->getHash() !== $this->getHash()) {
            ...
            if ($defPlural && $ourPlural) {
                ...
                $this["prev"][] = $ourPlural->exportBasic() + [
                    "parent" => 0,
                    "plural" => 1,
                ];
...
class LocoPoIterator implements Iterator, Countable
{
    ...
    public function __construct($po)
    {
        if (is_array($po)) {
            $this->po = $po;
        } elseif ($po instanceof Traversable) {
            $this->po = iterator_to_array($po, false);
        } else {
            throw new InvalidArgumentException(
                "PO data must be array or iterator"
            );
        }
        ...
        if (
            "" !== $h["source"] ||
            (isset($h["context"]) && "" !== $h["context"]) ||
            (isset($po[1]["parent"]) && 0 === $po[1]["parent"])
        ) {
            $this->z = -1;
        }
    }
    ...
    public function getHeaders()
    {
        if (is_null($this->headers)) {
            ...
            if (0 === $this->z) {
                ...
                } elseif ($value instanceof LocoPoHeaders) {
                    $this->headers = $value;
    ...
    public function count()
    {
        if (is_null($this->size)) {
            ...
            if (!self::is_prime($this->size) || $this->size < 3) {
                throw new Exception(
                    "Size expected to be prime number above 2, got " .
                        $this->size
                );
            }
        }
        ...
        foreach ($msgids as $msgid) {
            ...
            if (array_key_exists($idx, $this->map)) {
                ...
                do {
                    ...
                    if ($hash_val === $idx) {
                        throw new Exception(
                            "Unable to find empty slot in hash table"
                        );
                    }
                    $idx %= $hash_tab_size;
...
class LocoMo
{
    ...
    public function compile()
    {
        ...
        if (
            $this->head->has("Plural-Forms") &&
            preg_match(
                "/^nplurals=(\\d)/",
                $this->head->trimmed("Plural-Forms"),
                $r
            )
        ) {
            $nplurals = (int) $r[1];
...
class LocoTokenizer implements LocoTokensInterface
{
    ...
    public function define($grep, $t = 0)
    {
        if ("^" !== $grep[1]) {
            throw new InvalidArgumentException(
                "Expression " . $grep . ' isn\'t anchored'
            );
        }
        if (!is_int($t) && !is_callable($t)) {
            throw new InvalidArgumentException(
                "Non-integer token must be valid callback"
            );
        }
        $sniff = $grep[2];
...
function loco_utf8_chr($u)
{
    ...
    if ($u < 0x10000) {
        return chr((($u >> 12) & 15) | 0xe0) .
            chr((($u >> 6) & 0x3f) | 0x80) .
            chr(($u & 0x3f) | 0x80);
    }
    if ($u < 0x110000) {
        return chr((($u >> 18) & 7) | 0xf0) .
            chr((($u >> 12) & 0x3f) | 0x80) .
            chr((($u >> 6) & 0x3f) | 0x80) .
            chr(($u & 0x3f) | 0x80);
    }
...
function loco_resolve_surrogates($s)
{
    return preg_replace_callback(
        ...
        "_loco_resolve_surrogates",
        $s
    );
}
function _loco_resolve_surrogates(array $r)
{
    return loco_utf8_chr(
        ((((832 | (ord($r[1]) & 0x3f)) << 6) | (ord($r[2]) & 0x3f)) - 0xd800) *
            0x400 +
            ((((832 | (ord($r[3]) & 0x3f)) << 6) | (ord($r[4]) & 0x3f)) -
                0xdc00) +
            0x10000
    );
}
class LocoEscapeParser
{
    ...
    public function __construct(array $map = [])
    {
        ...
        if (!isset($map["u"])) {
            $rules[] =
                "u(?:\\{[0-9A-Fa-f]+\\}|[0-9A-Fa-f]{1,4})(?:\\\\u(?:\\{[0-9A-Fa-f]+\\}|[0-9A-Fa-f]{1,4}))*";
        }
    ...
    final public function unescape($s)
    {
        if ("" !== $s) {
            return $this->stripSlashes(
                preg_replace_callback($this->grep, [$this, "unescapeMatch"], $s)
            );
        }
        return "";
    }
...
class LocoJsTokens extends LocoTokenizer
{
    ...
    public function __construct($src = "")
    {
        ...
        $this->define(
            '/^(?:\\\\u[0-9A-F]{4,4}|[$_\\pL\\p{Nl}])(?:\\\\u[0-9A-F]{4}|[$_\\pL\\pN\\p{Mn}\\p{Mc}\\p{Pc}])*/ui',
            [$this, "matchWord"]
        );
...
interface LocoExtractorInterface
{
    ...
    public function extract(
        LocoExtracted $strings,
        LocoTokensInterface $tokens,
        $fileref = ""
    );
    ...
    protected function push(
        ...
        if ("" !== $comment) {
            if (
                ...
                $entry["format"] = $format;
                $comment = trim(
                    substr_replace($comment, "", $r[0][1], strlen($r[0][0]))
                );
            }
            if (
                preg_match(
...
class LocoPHPExtractor extends LocoExtractor
{
    ...
    public function fsniff($str)
    {
        ...
        while (
            ...
            $offset += strlen($match);
            if (
                preg_match("/^% +[a-z]/i", $match) ||
                preg_match("/^%[b-ou-x]/i", $match)
            ) {
                continue;
            }
            $format = "php";
    ...
    protected function comment($comment)
    {
        return preg_replace(
            "/^translators:\\s+/mi",
            "",
            loco_parse_php_comment($comment)
        );
    }
        ...
        while ($tok = $tokens->advance()) {
            ...
            if ($depth) {
                if (")" === $s || "]" === $s) {
                    if (0 === --$depth) {
                        if (
                            $this->push($strings, $rule, $args, $comment, $ref)
                        ) {
                            $n++;
                        }
                        $comment = "";
                    ...
                    if ("," === $s) {
                        $narg++;
                    } elseif (T_CONSTANT_ENCAPSED_STRING === $t) {
                        $s = self::utf8($s);
                        $args[$narg] = $this->decapse($s);
                    } elseif (
                        T_STRING === $t &&
                        array_key_exists($s, $this->defs)
                    ) {
            ...
            } elseif (T_COMMENT === $t || T_DOC_COMMENT === $t) {
                $was_header = false;
                $s = self::utf8($s);
                if (0 === $n) {
                    ...
                    if ($wp && ($header = loco_parse_wp_comment($s))) {
                        foreach ($wp as $domain => $tags) {
                            foreach (
                                array_intersect_key($header, $tags)
                                as $tag => $text
                            ) {
                                ...
                                if (is_string($meta)) {
                                    $meta = ["notes" => $meta];
                                    trigger_error(
                                        $tag . ' header defaulted to "notes"',
                                        E_USER_DEPRECATED
                                    );
                                }
                                $strings->pushEntry(
                                    ["source" => $text, "refs" => $ref] + $meta,
                                    (string) $domain
                                );
                                $was_header = true;
                            }
                ...
                "" !== $comment &&
                !preg_match("!^[/* ]+(translators|xgettext):!im", $comment)
            ) {
                $comment = "";
            }
        }
...
class LocoWpJsonExtractor implements LocoExtractorInterface
{
    ...
    private function getType($type)
    {
        ...
        if (!file_exists($path)) {
            throw new Exception(
                basename($path) . " not found in " . $this->base
            );
        }
    ...
    public function tokenize($src)
    {
        ...
        if (
            !preg_match(
                "!^https?://schemas.wp.org/trunk/(block|theme)\\.json!",
                $raw['$schema'],
                $r
            )
        ) {
...
class LocoWpJsonStrings extends ArrayIterator implements LocoTokensInterface
{
    ...
    private function walk($tpl, $raw)
    {
        ...
        } elseif (is_object($tpl) && is_array($raw)) {
            $group_key = "*";
            foreach ($raw as $key => $value) {
                if (isset($tpl->$key)) {
                    $this->walk($tpl->$key, $value);
                } elseif (isset($tpl->$group_key)) {
...
function loco_wp_extractor($type = "php", $ext = "")
{
    ...
    if ("php" === $type) {
        return substr($ext, -9) === "blade.php"
            ? new LocoBladeExtractor($rules)
            : new LocoPHPExtractor($rules);
    }
    if ("js" === $type) {
...
class LocoFuzzyMatcher implements Countable
{
    ...
    public function setFuzziness($s)
    {
        if ($this->po) {
            throw new LogicException(
                "Cannot setFuzziness() after calling match()"
            );
        }
    ...
    public function getFuzzyMatches()
    {
        ...
        foreach ($this->diff as $pair) {
            list($old, $new) = $pair;
            if (
                !array_key_exists($new, $this->pot) ||
                !array_key_exists($old, $this->po)
            ) {
                continue;
            }
